...
// Этот сервис позволяет подготавливать конфиг для графиков, рисуемых в интерфейсе.
// В зависимости от типа графика выбирается функция, в которой определенным образом подготавливаются данные и стили
export interface IChartConfigData {
  chartName: string;
  chartType: string;
  data: any;
}
...
export class ChartOptionService {
  ...
  configChart(data: IChartConfigData): any {
    console.log(data.chartType, this.chartTypeDict[data.chartType]);
    ...
    if (this.chartTypeDict[data.chartType] === 'line') {
      // необходимо для начала доработать createLine, чтобы работало с форматом данных, которые мы получаем при загрузке дашбордов
  ...
  createBar(data: IChartConfigData) {
    ...
    let sortedData = data.data.data.slice().sort(function (a: any, b: any) {
      a = a[Object.keys(a)[0]];
      b = b[Object.keys(b)[0]];
    ...
    sortedData.forEach((element: any, index: number) => {
      if (element[Object.keys(element)[0]]) {
        ...
        if (planDataSorted) {
          if (
            Math.round((element[Object.keys(element)[2]] / planDataSorted[index].y) * 100) > 85 &&
            Math.round((element[Object.keys(element)[2]] / planDataSorted[index].y) * 100) < 100
          ) {
        ...
        seriesData.push({
          ...
          valueText: Object.keys(element)[2],
          itemStyle: {
    ...
    if (planDataSorted) {
      ...
      chartOption.tooltip = {
        ...
        formatter: (obj: any) => {
          ...
          let value = obj.value;
          let index = obj.dataIndex;
          let valueText = obj.data.valueText;
          return (
            '<b>Дата: </b>' +
            name +
            '<br>' +
  ...
  createStackedBar(data: IChartConfigData) {
    ...
    let barStackedData = data.data.data.length
      ? this.groupBy(data.data.data, Object.keys(data.data.data[0])[1])
      : {};
    Object.keys(barStackedData).forEach((element) => {
      barStackedData[element] = barStackedData[element].slice().sort(function (a: any, b: any) {
        a = a[Object.keys(a)[0]];
        b = b[Object.keys(b)[0]];
        return a - b;
      });
    ...
    Object.keys(barStackedData).forEach((element) => {
      stackedSeries.push({
        ...
        data: barStackedData[element].map((elem: any) => {
          return elem[Object.keys(elem)[2]];
        }),
      });
      eleNumber++;
    });
    ...
    chartOption.xAxis = {
      type: 'category',
      data: barStackedData[Object.keys(barStackedData)[0]]?.map((elem: any) => {
        return new Date(elem[Object.keys(elem)[0]]).toLocaleDateString('ru-RU', {
          month: '2-digit',
          day: '2-digit',
        });
    ...
    if (this.chartTypeDict[data.chartType] === 'bar_stacked_plan') {
      // let planDataSorted = data.data.annotation_data[Object.keys(data.data.annotation_data)[0]][0].values.slice().sort(function (a: any, b: any) {
      //   a = a.x;
      //   b = b.x;
      //   return a - b;
      // });
      ...
      // let smoothness = data.chartName[data.chartName.length - 1] === 'я' ? true : false;
  ...
  createBigNumber(data: IChartConfigData) {
    let bigNumberObject = data.data.data[0];
    let bigNumberData = {
      title: data.chartName,
      data: bigNumberObject[Object.keys(bigNumberObject)[0]],
    };
      ...
      bubbleSeries.push({
        ...
        itemStyle: {
          ...
          color: new echarts.graphic.RadialGradient(0.4, 0.3, 1, [
            {
              offset: 0,
              color: this.colorPaletteSankey[index],
            },
            //  {
            //     offset: 1,
            //     color: '#616161'
  ...
  groupBy(array: any, key: string) {
    return array.reduce((result: any, currentValue: any) => {
      if (currentValue[key] === null || currentValue[key] === undefined) {
        return result;
      }
